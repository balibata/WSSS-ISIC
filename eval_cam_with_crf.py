...
def crf(n_jobs, is_coco=False, is_isic=False):
    """
    ...
    def process(i):
        if is_isic:
            ...
            print(f"Image shape before CRF: {image.shape}")  # Debugging
...
if __name__ == "__main__":
    ...
    is_isic = 'isic' in args.cam_out_dir
    ...
    elif 'isic' in args.cam_out_dir:
        # Assuming ISIC dataset has a folder structure
        # Image files in args.image_root and corresponding masks in args.gt_root
        eval_list = []
        for root, dirs, files in os.walk(args.image_root):
            for file in files:
                if file.endswith(".jpg"):  # Collect all JPG images
                    eval_list.append(os.path.splitext(file)[0])  # Save file name without extension
